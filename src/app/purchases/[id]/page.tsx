...
export default function PurchaseDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const purchase = useMemo(() => purchases.find(p => p.id === resolvedParams.id), [purchases, resolvedParams.id]);
  const assignments = useMemo(() => allAssignments.filter(a => a.lotId === resolvedParams.id), [allAssignments, resolvedParams.id]);
  ...
  if (!mounted || !isLoaded || !purchase || !distribution) return null;
      ...
      <main className="container px-4 max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between no-print">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="outline" className="gap-2" onClick={() => router.push(`/purchases/${purchase.id}/edit`)}><Pencil className="w-4 h-4" /> Edit Order</Button>
            <DeleteLotModal 
